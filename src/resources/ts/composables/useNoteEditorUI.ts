...
marked.use({
    renderer: {
        link({ href, title, text }) {
            const fixedHref = /^https?:\/\//.test(href)
                ? href
                : `https://${href}`;
...
export function useNoteEditorUI(options: {
        ...
        return lines
            .map((line, i) =>
                allHavePrefix
                    ...
                    : prefix.test(line)
                      ? line
                      : `${line.match(/^(\s*)/)?.[1] ?? ''}${type === 'unordered_list' ? '- ' : `${i + 1}. `}${line.trimStart()}`
            )
            .join('\n');
    }
    ...
    function applyFormat(
        ...
        if (type === 'link') {
            ...
            const formattedText = match ? match[1] : `${start}${text}${end}`;
    ...
    function stripAiMarkers(html: string) {
        ...
        div.querySelectorAll('[data-ai-parent], [data-ai-child]').forEach(
            (el) => {
                el.querySelectorAll('[data-ai-retranslate]').forEach((button) =>
                    button.remove()
                );
    ...
    function moveOutsideAiBlock() {
        ...
        const aiBlock = el.closest(
            '[data-ai-parent], [data-ai-child]'
        ) as HTMLElement | null;
    ...
    function insertHtmlOutsideElement(
        ...
        target.insertAdjacentHTML(
            mode === 'before' ? 'beforebegin' : 'afterend',
            html
        );
    }
    ...
    function insertAiResult(mode: InsertMode) {
        ...
        const parentHtml = createAiBlockHtml({
            ...
            aiIndex,
            text:
                aiAction.value === 'distant writing'
                    ? '<!--[Inizio commento] ' +
                      selected +
                      ' [Fine commento]-->'
                    : selected,
        ...
        const childHtml = createAiBlockHtml({
            ...
            lang:
                aiAction.value === 'translate' ? languageMode.value : undefined,
            sourceChildId:
                aiAction.value === 'translate' ? sourceChildId : undefined,
        });
        ...
        if (childMarker) {
            ...
            if (id) {
                ...
                translatedChildren?.forEach((child) => {
                    ...
                    const button = translatedChild.querySelector(
                        '[data-ai-retranslate]'
                    ) as HTMLElement | null;
                    button?.classList.remove('hidden');
                });
    ...
    async function retranslateAiBlock(child: HTMLElement) {
        ...
        if (sourceChildId) {
            ...
            const content = sourceChild.querySelector(
                '[data-ai-content]'
            ) as HTMLElement | null;
